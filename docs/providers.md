...
Browser cookie imports are cached in Keychain (`com.steipete.tokenbar.cache`, account `cookie.<provider>`) and reused
...
## OpenRouter
- API token from `~/.tokenbar/config.json` (`providerConfig.openrouter.apiKey`) or `OPENROUTER_API_KEY` env var.
